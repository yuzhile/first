...
def insertFeature(features,data,action):
	if features.has_key(data[1]):
		features[data[1]][30-data[-1]] = action[data[2]] + features[data[1]][30-data[-1]] 
	else:
		time = [0 for i in range(31)]
		time[30-data[-1]] = action[data[2]]
...
def genLabel(features,predict):
	...
	return label
def output(user_id,features,lable,r_writer,l_writer):
	keys = features.keys()
	i = 0
	for key in keys:
		temp = []
...
def extration(curor,predict_file,result_file,label_file):
	...
	#	print data
		if cur != pre:
			...
			output(data[0],features,label,r_writer,l_writer)
			features = {}	
			userinfo = []
			buyitem = []
			label = []
...
if __name__ == '__main__':
	command = 'select * from train_user order by user_id ,item_id limit 10000'
